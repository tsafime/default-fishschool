...
export class AclService implements ConfigData {
	...
	constructor(
		...
		this.authService.getUserRoles().subscribe(roles => {
			this.setCurrrentUserRole(roles);
		});
		...
		this.authService.onCredentialUpdated$
			...
			.subscribe(roles => this.setCurrrentUserRole(roles));
	...
	setCurrrentUserRole(role: any): any {
		// update role if the credential data has role
		if (role != null) {
